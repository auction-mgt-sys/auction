...
Class Action {
	...
	function save_payment(){
		...
		if(empty($transaction_id) || empty($amount) || empty($reason)){
			return 0;
			exit;
		}
		if(empty($id)){
	...
	//verify the payment by adding amount 
	...
	function Verify_comment(){
		...
		if($save){
			$delete = $this->db->query("DELETE FROM payment where id = ".$pid);
			if ($delete) {
				# code...
				return 1;
			}
	...
	function bid_Verify_comment(){
		...
		//$delete = $this->db->query("DELETE FROM comment where id = ".$id);
		if($save){
			$delete = $this->db->query("DELETE FROM bids where id = ".$pid);
			if ($delete) {
				# code...
				return 1;
			}
	...
	function cancel_bid_Verify_comment(){
		extract($_POST);
		$sender_id = $_SESSION['login_id'];
		$user_type = $_SESSION['login_type'];
		$data = " sender_id = '$sender_id' ";
		...
		//$delete = $this->db->query("DELETE FROM comment where id = ".$id);
		if($save){
			$save2 = $this->db->query("UPDATE bids set status = 3 where id = ".$pid);
			if ($save2) {
				# code...
				return 1;
			}
	...
	function delete_bid(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM bids where id = ".$id);
		if($delete)
			return 1;
	}
	function winner(){
		extract($_POST);
		$save = $this->db->query("UPDATE bids set status = 2 where id = ".$id);
		if($save)
			return 1;
	}
	function bid_cancl(){
		extract($_POST);
		$save = $this->db->query("UPDATE bids set status = 3 where id = ".$id);
		if($save)
			return 1;
	}
	function delet_payment(){
		extract($_POST);
