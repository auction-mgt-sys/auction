...
Class Action {
	...
	function reject(){
		extract($_POST);
		$save = $this->db->query("UPDATE users set status = 2 where id = ".$id);
		if($save)
			return 1;
	}
	//verify comment here
	function user_reject_verify(){
		extract($_POST);
		...
		if($save){
			$reject = $this->db->query("UPDATE users set status = 2 where id = ".$pid);
			if ($reject) {
				# code...
				return 1;
			}
	...
	function accept(){
		extract($_POST);
		$save = $this->db->query("UPDATE users set status = 1 where id = ".$id);
		if($save)
			return 1;
	}
